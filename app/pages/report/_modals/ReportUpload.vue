...
const handleExampleSelect = (example: string) => {
  if (fileStore.selectedReportExample === example) {
    fileStore.setReportExample(null);
  } else {
    fileStore.setReportExample(example);
  }
};
...
const handleSubmit = async () => {
  if (props.onSubmit) {
    await props.onSubmit({
      selectedExample: fileStore.selectedReportExample,
      selectedFile: fileStore.selectedReportFile,
    });
  }
  // 모달 닫기
  ...
  <Modal
    ...
    :buttons="[
      ...
      },
      {
        ...
        disabled: fileStore.selectedReportExample == null && fileStore.selectedReportFile == null,
      },
    ]"
    @close="
      () => {
  ...
  >
    ...
    <div class="mt-[30px] flex flex-col gap-[40px]">
      ...
      <label
        ...
        :class="[fileStore.selectedReportFile ? 'justify-between' : 'justify-center']"
      >
        <span
          :class="[
            'overflow-hidden text-ellipsis whitespace-nowrap',
            fileStore.selectedReportFile
              ? 'w-96 text-[24px] font-[400] text-foreground'
              : 'text-[28px] font-[600] text-gray-b4',
          ]"
        >
          {{
            fileStore.selectedReportFile ? fileStore.selectedReportFile.name : "직접 파일 불러오기"
          }}
        </span>
        <TrashIcon
          v-if="fileStore.selectedReportFile"
      ...
      <input id="report-upload" type="file" class="hidden" @change="handleFileChange" />
    </div>
  </Modal>
</template>
